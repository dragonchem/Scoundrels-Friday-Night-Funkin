...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'life-will-change' | 'take over' | 'dance':
			{
					defaultCamZoom = 0.9;
					curStage = 'kittenstage';
			...
			case 'cammy' | 'claw' | 'boxer':
			{
					defaultCamZoom = 0.9;
					curStage = 'b0lletstage';
					var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('stageback'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
		...
		var kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + " - KE " + MainMenuState.kadeEngineVer + " - " + (FlxG.save.data.etternaMode ? "E.Mode" : "vsSCDRL"), 16);
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet, daNoteData, oldNote, true, SONG.speed);
